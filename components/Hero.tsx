...
interface DeclaredFile {
  ...
  answerLink: string;
  guideLink: string;
  password?: string; // Mật khẩu riêng cho từng tài liệu
}
...
const Hero: React.FC<HeroProps> = ({ onStart }) => {
  ...
  // Lưu trữ tài liệu (localStorage cho bản demo)
  ...
  // Các trường trong Form Quản trị
  ...
  // State xử lý mở khóa tài liệu phía Học sinh
  ...
  // Hiệu ứng thay đổi số lượng thí sinh ngẫu nhiên để tăng tính trực quan
  useEffect(() => {
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-10 md:py-20 text-center">
      ...
      <div className="max-w-7xl mx-auto text-slate-300 text-sm md:text-xl font-medium mb-12 px-2">
        <p className="md:whitespace-nowrap">
          Giúp học sinh THPT học đúng trọng tâm – phát triển tư duy – tự tin chinh phục kỳ thi.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-6 lg:gap-8 max-w-6xl mx-auto">
        <div onClick={() => setShowFolders(true)} className="cursor-pointer h-full">
          <FeatureCard 
            ...
            desc="Cập nhật mới nhất theo cấu trúc Bộ Giáo dục & Đào tạo."
            visitors={counts.kho}
          />
        </div>
        <div onClick={handleLuyenDeClick} className="cursor-pointer h-full">
          <FeatureCard 
            icon="🌏"
            title="ĐỀ THI THỬ TN THPT MÔN HÓA HỌC- 2026"
            subtitle="Đề thi thử TN PTTH trên toàn quốc"
            desc="Làm bài trực tuyến tại hệ thống khảo thí quốc gia của VANHAI."
            visitors={counts.luyenDe}
          />
        </div>
        <div onClick={() => setShowGuideModal(true)} className="cursor-pointer h-full">
          <FeatureCard 
            ...
            desc="Cẩm nang hướng dẫn sử dụng Kho tài liệu và Luyện đề thông minh 24/7."
            visitors={counts.huongDan}
          />
        </div>
      </div>
            ...
            {!selectedFolder ? (
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                ...
                {portView === 'selection' && (
                  <div className="flex-1 flex flex-col justify-center gap-6 animate-in zoom-in-95">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 w-full max-w-2xl mx-auto">
                      <button 
                        onClick={() => setPortView('student')}
                        className="p-10 bg-gradient-to-br from-teal-800 to-teal-900 border-2 border-teal-500/30 rounded-[2.5rem] hover:border-[#D4AF37] transition-all group shadow-xl"
                      >
                      ...
                      <button 
                        onClick={() => setPortView('admin-login')}
                        className="p-10 bg-gradient-to-br from-slate-800 to-slate-900 border-2 border-slate-500/30 rounded-[2.5rem] hover:border-[#D4AF37] transition-all group shadow-xl"
                      >
                        <div className="text-6xl mb-4 group-hover:scale-110 transition-transform">🔐</div>
                ...
                {portView === 'admin-login' && (
                  <div className="max-w-md mx-auto w-full py-12 animate-in slide-in-from-bottom-4">
                    <h4 className="text-2xl font-black text-white mb-8 text-center uppercase tracking-widest">Xác thực quyền hạn</h4>
                    <div className="space-y-6">
                      ...
                      />
                      {adminError && <p className="text-red-400 text-xs font-bold text-center">Mật khẩu không chính xác!</p>}
                      <button 
                        onClick={handleAdminLogin}
                        className="w-full py-5 bg-[#D4AF37] text-[#003d3d] font-black rounded-2xl hover:bg-[#FCF6BA] transition-all shadow-xl uppercase tracking-widest"
                      >
                        Đăng Nhập
                      </button>
                    </div>
                ...
                {portView === 'admin-dashboard' && (
                  <div className="flex-1 flex flex-col overflow-hidden animate-in fade-in">
                    <div className="bg-[#003d3d] p-6 rounded-3xl border border-[#006666] mb-8">
                      <h4 className="text-lg font-black text-white mb-6 uppercase tracking-wider flex items-center gap-2">
                        <span className="text-[#D4AF37]">✚</span> Khai báo tài liệu mới
                      </h4>
                      ...
                      <button onClick={handleAddFile} className="mt-6 w-full py-4 bg-teal-600 hover:bg-teal-500 text-white font-black rounded-xl transition-all shadow-lg uppercase tracking-widest">
                        Xác nhận khai báo
                      </button>
                    </div>

                    <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
                      <h4 className="text-xs font-black text-teal-400 mb-4 uppercase tracking-[0.2em]">Danh sách đã khai báo</h4>
                      <div className="space-y-3">
                        {(declaredFiles[selectedFolder.id] || []).map((file) => (
                          <div key={file.id} className="flex items-center justify-between p-4 bg-black/20 border border-[#006666] rounded-2xl group">
                ...
                {portView === 'student' && (
                  <div className="flex-1 overflow-y-auto custom-scrollbar animate-in slide-in-from-right-4 pr-2">
                    ...
                    <div className="grid grid-cols-1 gap-4 pb-6">
                      {(declaredFiles[selectedFolder.id] || []).map((file) => (
                        <div key={file.id} className="bg-[#003d3d]/80 border border-[#006666] p-6 rounded-3xl hover:border-[#D4AF37] transition-all shadow-xl">
                          ...
                          {file.password && !unlockedFiles.has(file.id) ? (
                            <div className="bg-black/20 p-6 rounded-2xl border border-yellow-600/30">
                              <p className="text-yellow-500 text-xs font-black uppercase mb-4 text-center">Tài liệu này yêu cầu mật khẩu riêng</p>
                              <div className="flex flex-col sm:flex-row gap-2">
                                <input 
                                  type="password" 
                                  placeholder="Nhập mật khẩu tài liệu" 
                                  className="flex-1 p-3 bg-teal-900/40 border border-teal-700 rounded-xl text-white outline-none focus:border-yellow-500 text-center"
                                  value={studentPasswordInput[file.id] || ''}
                                  onChange={(e) => setStudentPasswordInput(prev => ({...prev, [file.id]: e.target.value}))}
                                  onKeyDown={(e) => e.key === 'Enter' && handleUnlockFile(file)}
                                />
                                <button 
                                  onClick={() => handleUnlockFile(file)}
                                  className="px-6 py-3 bg-yellow-600 text-black font-black rounded-xl hover:bg-yellow-500 transition-all uppercase text-xs"
                                >
                                  MỞ KHÓA
                                </button>
                              </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                              <a href={file.examLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 p-3 bg-teal-900/50 text-teal-300 border border-teal-500/30 rounded-xl font-bold text-xs hover:bg-teal-700 hover:text-white transition-all uppercase">
                                📥 Link Đề
                              </a>
                              <a href={file.answerLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 p-3 bg-blue-900/30 text-blue-300 border border-blue-500/30 rounded-xl font-bold text-xs hover:bg-blue-700 hover:text-white transition-all uppercase">
                                ✅ Link Đáp án
                              </a>
                              <a href={file.guideLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 p-3 bg-[#D4AF37]/10 text-[#D4AF37] border border-[#D4AF37]/30 rounded-xl font-bold text-xs hover:bg-[#D4AF37] hover:text-[#003d3d] transition-all uppercase text-center">
                                💡 Giải chi tiết
                              </a>
                            </div>
                          )}
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center px-4 bg-black/95 backdrop-blur-xl overflow-y-auto py-10">
          <div className="max-w-4xl w-full bg-[#004d4d] border border-[#D4AF37]/30 rounded-[2.5rem] p-6 md:p-10 shadow-[0_0_60px_rgba(212,175,55,0.15)] relative text-left max-h-[90vh] overflow-y-auto custom-scrollbar">
            <button 
              ...
              className="sticky top-0 self-end float-right -mt-2 -mr-2 w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full bg-[#D4AF37] text-[#003d3d] z-20 font-black shadow-lg hover:rotate-90 transition-all"
            >
              ✕
            </button>
            
            <h3 className="text-2xl md:text-4xl font-black text-white mb-10 uppercase tracking-widest border-b-4 border-[#D4AF37] pb-4 inline-block">
              CẨM NANG SỬ DỤNG
            </h3>
                ...
                <div className="space-y-6 md:pl-16">
                  <div>
                    ...
                    <ul className="text-slate-300 space-y-3 text-sm md:text-base ml-4">
                      <li className="flex gap-2"><span>•</span> Nhấn vào thẻ <b className="text-[#D4AF37]">KHO TÀI LIỆU PHONG PHÚ</b> tại trang chủ.</li>
                      <li className="flex gap-2"><span>•</span> Lựa chọn <b className="text-white">thư mục chuyên biệt</b> phù hợp với nhu cầu (Đề thi 2026, Đề Minh Họa, v.v.).</li>
                      <li className="flex gap-2"><span>•</span> Chọn <b className="text-[#D4AF37]">"CỔNG HỌC SINH"</b> để xem tài liệu công khai.</li>
                    </ul>
                  </div>

                  <div>
                    <p className="text-white font-black text-lg mb-3">Tính năng chính:</p>
                    <ul className="text-slate-300 space-y-3 text-sm md:text-base ml-4">
                      <li className="flex gap-2"><span>•</span> <div><b className="text-teal-400">Tải xuống miễn phí:</b> Tất cả file PDF, Docx đều được lưu trữ trên Google Drive tốc độ cao.</div></li>
                      <li className="flex gap-2"><span>•</span> <div><b className="text-teal-400">Cập nhật liên tục:</b> VanHai Education bổ sung tài liệu mới mỗi tuần theo sát lộ trình Bộ GD&ĐT.</div></li>
                    </ul>
                  </div>
                </div>
              </section>
              ...
              <section className="bg-teal-900/20 p-6 md:p-8 rounded-[2rem] border border-teal-500/10">
                ...
                <div className="space-y-6 md:pl-16">
                  ...
                  <ol className="text-slate-300 space-y-4 text-sm md:text-base ml-4 list-decimal">
                    ...
                    <li>Chọn đề thi mong muốn và nhấn <b className="text-white">"Làm Bài"</b>.</li>
                    <li>Hệ thống sẽ tự động <b className="text-red-400">đếm ngược thời gian</b>. Hãy hoàn thành các câu hỏi trắc nghiệm trước khi hết giờ.</li>
                    <li>Sau khi nộp bài, hệ thống sẽ <b className="text-green-400">chấm điểm tức thì</b>.</li>
                    <li>Học sinh xem được <b className="text-yellow-400">lỗi sai</b></li>
                    <li>Có <b className="text-red-500">cảnh báo gian lận</b>, Thí sinh vi phạm 3 lần <b className="text-red-600">sẻ loại ngay</b>.</li>
                    <li className="bg-[#D4AF37]/10 p-4 rounded-xl border border-[#D4AF37]/20">
                      Thí sinh muốn xem lời giải chi tiết liên hệ zalo:<b className="text-[#D4AF37] text-lg">0937648777</b>
                    </li>
                  </ol>
                </div>
              </section>
          ...
          <div className="max-w-4xl w-full bg-[#004d4d] border border-[#D4AF37]/30 rounded-[2.5rem] p-6 md:p-8 shadow-2xl relative flex flex-col items-center max-h-[90vh] overflow-y-auto">
            ...
            <div className="w-full text-center py-10">
              ...
              <div className="flex flex-col sm:flex-row gap-6 justify-center max-w-2xl mx-auto">
                <a href="https://drive.google.com/file/d/1VCOzlAo0YAykCBrpGmFTvepVzweV4M_1/view?usp=sharing" target="_blank" rel="noopener noreferrer" className="flex-1 p-10 bg-[#003d3d] border border-[#006666] rounded-3xl hover:border-[#D4AF37] transition-all group shadow-xl">
                  <div className="text-6xl mb-4">🎓</div>
                  <p className="text-lg font-black text-white uppercase mb-1">CỔNG HỌC SINH</p>
                  <p className="text-teal-400 text-[10px] font-black uppercase">Tài liệu miễn phí</p>
                </a>
                <div onClick={handleLuyenDeClick} className="flex-1 p-10 bg-[#003d3d] border border-[#006666] rounded-3xl hover:border-[#D4AF37] transition-all group shadow-xl cursor-pointer">
                  <div className="text-6xl mb-4">🌏</div>
                  <p className="text-lg font-black text-white uppercase mb-1">ĐỀ THI THỬ TN THPT MÔN HÓA HỌC- 2026</p>
                  <p className="text-teal-300 text-[10px] font-black uppercase">Vào hệ thống thi chính thức</p>
                </div>
                <div onClick={() => { setShowFolders(true); setShowWelcomeModal(false); }} className="flex-1 p-10 bg-[#003d3d] border border-[#006666] rounded-3xl hover:border-[#D4AF37] transition-all group shadow-xl cursor-pointer">
                  <div className="text-6xl mb-4">📂</div>
                  <p className="text-lg font-black text-white uppercase mb-1">KHO TÀI LIỆU</p>
                  <p className="text-[#D4AF37] text-[10px] font-black uppercase">Hệ thống thư mục</p>
                </div>
              </div>
            </div>
          </div>
...
const FeatureCard: React.FC<{icon: string, title: string, subtitle?: string, desc: string, visitors?: number}> = ({ icon, title, subtitle, desc, visitors }) => {
  return (
    <div className="p-6 md:p-8 h-full bg-[#004d4d]/60 border border-teal-500/20 rounded-[2.5rem] shadow-[0_15px_35px_rgba(0,0,0,0.5)] hover:shadow-[0_25px_50px_rgba(0,0,0,0.7)] hover:-translate-y-3 transition-all duration-500 group flex flex-col items-center min-h-[400px] md:min-h-[440px] relative overflow-hidden backdrop-blur-md border-t-teal-400/20">
      ...
      {/* Hiển thị số thí sinh đăng nhập */}
      {visitors !== undefined && (
        <div className="mb-4 px-3 py-1 bg-black/40 border border-teal-500/30 rounded-full flex items-center gap-2">
          ...
          <span className="text-[10px] md:text-xs font-bold text-teal-300 uppercase tracking-wider">
            Số thí sinh đăng nhập: <span className="text-white">{visitors.toLocaleString()}</span>
          </span>
        </div>
      )}
